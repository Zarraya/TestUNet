...
public struct Matrix3 {
  ...
  public static bool IsAffine(Matrix3 m) {
    ...
    return System.Math.Abs(Vector3.LengthSquared(v0 - v1)) < 0.00001f;
  }
  ...
  public static void Decompose(Matrix3 m,
    ...
    rotateY = -(float)System.Math.Asin(-mtx[0][2]);

    if (System.Math.Abs(System.Math.Cos(rotateY)) >= 0.00001f) {
      rotateX = (float)System.Math.Atan2(mtx[1][2], mtx[2][2]);
      rotateZ = (float)System.Math.Atan2(mtx[0][1], mtx[0][0]);
    }
    else {
      rotateX = (float)System.Math.Atan2(mtx[1][0], mtx[1][1]);
      rotateZ = 0.0f;
    }
  }
  ...
  public static void ToEulerZXY(Matrix3 m, out float x, out float y, out float z) {
    if (m[2][1] < 1.0f) {
      if (m[2][1] > -1.0f) {
        x = (float)System.Math.Asin(-m[2][1]);
        z = (float)System.Math.Atan2(m[0][1], m[1][1]);
        y = (float)System.Math.Atan2(-m[2][0], m[2][2]);
      }
      else {
        // The solution is not unique. Choosing z = 0.
        x = (float)System.Math.PI * 0.5f;
        z = 0.0f;
        y = (float)System.Math.Atan2(m[0][2], m[0][0]);
      }
    } else {
