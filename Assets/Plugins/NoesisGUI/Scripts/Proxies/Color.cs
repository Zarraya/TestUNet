...
public struct Color {
  ...
  private static float Gamma(float v)
  {
    if (v <= 0.00304f) {
      ...
      return 1.055f * (float)System.Math.Pow((double)v, 1.0 / 2.4) - 0.055f;
    }
  }
  ...
  private static float DeGamma(float v) {
    if (v <= 0.03928f) {
      ...
      return (float)System.Math.Pow((v + 0.055) / 1.055, 2.4);
    }
  }
  ...
  private static byte ToByte(float v) {
    return (byte)System.Math.Round(System.Math.Max(0.0f, System.Math.Min(v * 255.0f, 255.0f)));
  }
  ...
  public override string ToString() {
    return System.String.Format("#{0:X8}", PackedBGRA);
  }
