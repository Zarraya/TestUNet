...
public struct Rect {
  ...
  public static Rect Infinite {
    get {
      return new Rect(
        (float)System.Double.NegativeInfinity, (float)System.Double.NegativeInfinity,
        (float)System.Double.PositiveInfinity, (float)System.Double.PositiveInfinity);
    }
  }
  ...
  public override string ToString() {
    return System.String.Format("{0},{1},{2},{3}", X, Y, Width, Height);
  }
  ...
  public Rect Intersect(Rect rect) {
    ...
    float w = System.Math.Max(0.0f, (rect.Right > Right ? (rect.Left < Right ? Right : 0.0f) : (rect.Right > Left ? rect.Right : 0.0f)) - x);
    float h = System.Math.Max(0.0f, (rect.Bottom > Bottom ? (rect.Top < Bottom ? Bottom : 0.0f) : (rect.Bottom > Top ? rect.Bottom : 0.0f)) - y);
    if (w == 0.0f || h == 0.0f) {
      return Rect.Empty;
    }
    else {
  ...
  public void Expand(Point point) {
    ...
    Width = System.Math.Max(right, point.X) - X;
    Height = System.Math.Max(bottom, point.Y) - Y;
  }

  public void Expand(Rect rect) {
    float right = Right;
    float bottom = Bottom;
    X = System.Math.Min(X, rect.X);
    Y = System.Math.Min(Y, rect.Y);
    Width = System.Math.Max(right, rect.Right) - X;
    Height = System.Math.Max(bottom, rect.Bottom) - Y;
  }

  public void Transform(Transform2 m) {
    if (!System.Double.IsInfinity(Width) && !System.Double.IsInfinity(Height)) {
      Point tl = TopLeft * m;
      Point tr = TopRight * m;
