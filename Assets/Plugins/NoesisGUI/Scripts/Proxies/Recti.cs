...
public struct Recti {
  ...
  public Recti Intersect(Recti rect) {
    ...
    uint w = (uint)System.Math.Max(0.0f, (rect.Right > Right ? (rect.Left < Right ? Right : 0) : (rect.Right > Left ? rect.Right : 0)) - x);
    uint h = (uint)System.Math.Max(0.0f, (rect.Bottom > Bottom ? (rect.Top < Bottom ? Bottom : 0) : (rect.Bottom > Top ? rect.Bottom : 0)) - y);
    if (w == 0 || h == 0) {
      return Recti.Empty;
    }
    else {
  ...
  public void Expand(Recti rect) {
    ...
    Width = (uint)(System.Math.Max(right, rect.Right) - X);
    Height = (uint)(System.Math.Max(bottom, rect.Bottom) - Y);
  }
